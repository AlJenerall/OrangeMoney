...
2. **Third-Party Payment Integration**:
    ...
    - Includes a sample integration with **Orange Money** using the provided Swagger file.
...
Create `config/orange_money.php` with your Orange Money API credentials:
...
```
These values are used by `OrangeMoneyGateway` to initiate transactions and fetch their status.
